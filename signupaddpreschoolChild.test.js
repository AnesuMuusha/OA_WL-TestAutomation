...
;(async () => {
  ...
  try {
    ...
    await page.fill('input[placeholder="e.g. Nothando_123"]', 'ProgressSummaryOA0013')
...
await page.locator('p.text-sm.font-h1.font-normal.text-white', { hasText: 'Confirm' }).click(); 
    ...
    await page.fill('input[placeholder="First name"]', 'ProgressSummaryOA0013');

    // Click "Enter passport number" button
    await page.click('p.font-semibold.text-xs:has-text("Enter passport number instead")');

    //Passport:
    await page.fill('input[placeholder="e.g. A012345"]', 'ProgressSummaryOA0013');
