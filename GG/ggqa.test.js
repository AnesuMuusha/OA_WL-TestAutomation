...
const START_ID = 201 // IDs 33-200 already exist in the target system
...
function nextId() {
  ...
  return `ID${String(current).padStart(5, "0")}`
}
...
async function runRegistrationWizard(invitePage, { label, generatedId, CHW, namePrefix, surnamePrefix, extraStepHandler, photoQuestionChoice = "No", maxSteps = 20 }) {
  ...
  await invitePage.screenshot({ path: path.join(SCREENSHOTS_DIR, `stalled_${label.replace(/\s+/g, "_").toLowerCase()}.png`), fullPage: true }).catch(() => {})
...
async function recordVisitForFolder(invitePage, { personFirstName, resultLabel, screenshotPrefix, maxRounds = 8 }) {
  ...
  if ((await visitButton.count()) === 0) {
    ...
    await invitePage.screenshot({ path: path.join(SCREENSHOTS_DIR, `${screenshotPrefix}_no_visit_button.png`), fullPage: true }).catch(() => {})
    return false
  }
  ...
  if ((await startVisitBtn.count()) === 0) {
    ...
    await invitePage.screenshot({ path: path.join(SCREENSHOTS_DIR, `${screenshotPrefix}_no_start_visit.png`), fullPage: true }).catch(() => {})
    return false
  }
  ...
  for (let round = 1; round <= maxRounds; round++) {
    ...
    if (postActivityText.includes("Past visits")) {
      ...
      await invitePage.screenshot({ path: path.join(SCREENSHOTS_DIR, `${screenshotPrefix}_recorded.png`), fullPage: true }).catch(() => {})
  ...
  await invitePage.screenshot({ path: path.join(SCREENSHOTS_DIR, `${screenshotPrefix}_dashboard.png`), fullPage: true }).catch(() => {})
...
// Onboards its own CHW (same flow as the test above), then simulates the
// laptop-testing habit of switching off the Network tab by calling
// context.setOffline(true) — Playwright's equivalent of DevTools' "Offline"
// checkbox: every request the page makes is failed/blocked at the network
// layer rather than just throttled. With the app offline, this test opens its
// first folder — a Child WITH RTHB details — and records a visit for that
// child, all while offline, then reconnects only once the offline visit is
// done, to see whether the app syncs cleanly afterward.
test("GG/ggqa - offline: child with RTHB", async () => {
  ...
  try {
    ...
    const offlineCHW = {
      ...
      surname: offlineId,
      phoneNumber: "0834071970",
      passportNumber: offlineId,
      clinicLabel: "Clinic_0201",
      clinicValue: "3052ae47-8a61-4d0d-a4d5-c1dcd5b00d9e",
    }
    const offlineFullName = `${offlineCHW.firstName} ${offlineCHW.surname}`

    // ----- Steps 1-9: same admin login / CHW creation / device onboarding /
    // community join flow as the account-creation test above, so this test is
    // fully self-contained and doesn't depend on the other test having run. -----
    ...
    if (!emailFieldFound) throw new Error("Email input field not found with any selector")
    ...
    await page.type('input[name="firstName"]', offlineCHW.firstName)
    await page.click('input[name="surname"]')
    await page.type('input[name="surname"]', offlineCHW.surname)
    await page.waitForTimeout(1000)
    await page.fill('input[name="phoneNumber"]', offlineCHW.phoneNumber)
    await page.waitForTimeout(1000)
    ...
    await page.waitForFunction(
      ...
      offlineCHW.clinicLabel,
      { timeout: 15000 },
    )
    try {
      await page.selectOption(clinicSelect, offlineCHW.clinicValue)
    } catch (e) {
      await page.selectOption(clinicSelect, { label: offlineCHW.clinicLabel })
    }
    const selectedText = await page.$eval(clinicSelect, (el) => el.options[el.selectedIndex].textContent.trim())
    if (selectedText !== offlineCHW.clinicLabel) {
      throw new Error(`Clinic not selected correctly, got: ${selectedText}`)
    }
    console.log("CHW clinic selected:", selectedText)
    ...
    const invitationLinkLocator = page.locator("text=/https:\\/\\/growgreat-qa-api\\.azurewebsites\\.net\\/.+/").first()
    await invitationLinkLocator.waitFor({ state: "visible", timeout: 30000 })
    const invitationLink = (await invitationLinkLocator.textContent()).trim()
    console.log("Invitation Link:", invitationLink)

    invitePage = await context.newPage()
    await invitePage.goto(invitationLink, { waitUntil: "domcontentloaded" })
    await invitePage.locator('input[name="cellphone"]').waitFor({ state: "visible", timeout: 30000 })

    const invitePassportButton = invitePage.locator('p:text-is("Enter Passport number instead")')
    await invitePassportButton.waitFor({ state: "visible", timeout: 30000 })
    await invitePassportButton.click()

    const passportInput = invitePage.locator('input[name="username"]')
    await passportInput.waitFor({ state: "visible", timeout: 30000 })
    await passportInput.fill(offlineCHW.passportNumber)

    const cellphoneInput = invitePage.locator('input[name="cellphone"]')
    await cellphoneInput.waitFor({ state: "visible", timeout: 30000 })
    await cellphoneInput.fill(offlineCHW.phoneNumber)

    const passwordInput = invitePage.locator('input[name="password"]')
    await passwordInput.waitFor({ state: "visible", timeout: 30000 })
    await passwordInput.fill("Tester_12")

    await invitePage.locator('input[name="termsAndConditionsAccepted"]').check()
    await invitePage.locator('input[name="dataPermissionAgreementAccepted"]').check()
    ...
    const languageDropdown = invitePage.getByRole("button", { name: /Tap to choose language/i })
    ...
    await englishOption.waitFor({ state: "visible", timeout: 30000 })
    await englishOption.click()
    await page.waitForTimeout(5000)

    const nextButton = invitePage.getByRole("button", { name: "Next" })
    await nextButton.waitFor({ state: "visible", timeout: 30000 })
    await nextButton.click()

    const skipButton = invitePage.getByRole("button", { name: "Skip" })
    await skipButton.waitFor({ state: "visible", timeout: 30000 })
    await skipButton.click()
    await page.waitForTimeout(5000)
    await invitePage.waitForTimeout(5000)
    console.log("Account created, language selected — heading straight to the offline folder/visit (community join deferred to after)")

    // ----- While still ONLINE: use a real page.goto() this one time to get back
    // to the home screen and drill into the "Open a new folder" menu (folder-type
    // picker). We only flip to offline once we're sitting on that screen, so no
    // goto/reload is ever needed again afterwards — every step from here on is an
    // in-app click, same as a user mid-task who loses signal. -----
    await invitePage.goto("https://growgreat-qa-fe.azurewebsites.net/", { waitUntil: "domcontentloaded" })
    await invitePage.waitForTimeout(2000)
    ...
    await invitePage.waitForTimeout(2000)
    console.log("On the 'Open a new folder' menu (still online) — going offline now")

    // ----- Go offline: Playwright's context.setOffline(true) is the automated
    // equivalent of manually ticking "Offline" in DevTools' Network tab — it
    // fails every network request the page issues from this point on. From here
    // on, no page.goto()/reload/URL navigation is used — only in-app clicks,
    // since a hard navigation while offline renders a blank page (no
    // offline-capable page shell to fall back to). -----
    await context.setOffline(true)
    console.log("Context is now OFFLINE (context.setOffline(true))")
    await invitePage.waitForTimeout(1000)
    await invitePage.screenshot({ path: path.join(SCREENSHOTS_DIR, "offline_mode_started.png"), fullPage: true }).catch(() => {})

    // Offline-safe: picks the folder type from the menu we're already on and
    // starts the wizard — purely in-app clicks, no navigation of any kind.
    ...
    // Offline-safe visit recording for the folder we're already sitting on after
    // registration Save & Close (the new client's profile page) — deliberately
    // does NOT reuse recordVisitForFolder, since that helper starts with a
    // page.goto() back to the home URL to search for the folder by name, which
    // is exactly the reload that shows a blank page while offline. Here we stay
    // on the current page and only fall back to an in-app "Client folders" click
    // (never a goto) if the Visit button isn't already on screen.
    async function recordVisitOffline(invitePage, { personFirstName, resultLabel, screenshotPrefix, maxRounds = 8 }) {
      // Registration Save & Close drops us straight onto the Client folders list
      // (the "Clients" tab), which is itself a two-step tour popup — same pattern
      // used everywhere else in this suite. Must dismiss it before anything on
      // the page underneath is clickable.
      const foldersSkipTourBtn = invitePage.getByText("No, skip", { exact: true })
      if ((await foldersSkipTourBtn.count()) > 0 && (await foldersSkipTourBtn.isVisible().catch(() => false))) {
        await foldersSkipTourBtn.click().catch(() => {})
        console.log(`${resultLabel}: dismissed client folders tour popup (step 1)`)
        await invitePage.waitForTimeout(1000)
      }
      const foldersCloseTourBtn = invitePage.getByRole("button", { name: "Close" })
      if ((await foldersCloseTourBtn.count()) > 0 && (await foldersCloseTourBtn.isVisible().catch(() => false))) {
        await foldersCloseTourBtn.click().catch(() => {})
        console.log(`${resultLabel}: dismissed client folders tour popup (step 2)`)
        await invitePage.waitForTimeout(1000)
      }

      // Home > folders > find a folder > click on the folder. We're already on
      // the folders list, so "find a folder" is just locating this child's row.
      const folderRow = invitePage.getByText(personFirstName, { exact: false }).first()
      if ((await folderRow.count()) === 0) {
        console.log(`${resultLabel}: folder row for "${personFirstName}" not found on the Clients list`)
        await invitePage.screenshot({ path: path.join(SCREENSHOTS_DIR, `${screenshotPrefix}_no_folder_row.png`), fullPage: true }).catch(() => {})
        return false
      }
      await folderRow.click({ force: true }).catch(() => {})
      console.log(`${resultLabel}: opened folder for ${personFirstName}`)
      await invitePage.waitForTimeout(2000)

      const visitButton = invitePage.locator("button", { hasText: /visit/i }).first()
      if ((await visitButton.count()) === 0) {
        console.log(`${resultLabel}: no visit-related button found on the folder page`)
        await invitePage.screenshot({ path: path.join(SCREENSHOTS_DIR, `${screenshotPrefix}_no_visit_button.png`), fullPage: true }).catch(() => {})
        return false
      }
      await visitButton.click({ force: true }).catch(() => {})
    ...
    // ----- Folder 1 for this test: Child, WITH RTHB details, created OFFLINE -----
    const dob = new Date()
    ...
    const childOfflineResult = await runRegistrationWizard(invitePage, {
      label: "Child registration (RTHB, OFFLINE)",
      generatedId: offlineId,
      CHW: offlineCHW,
      namePrefix: "BabyOfflineRTHB",
      surnamePrefix: "Auto",
      extraStepHandler: childExtraHandler,
      photoQuestionChoice: "Yes",
    })

    if (childOfflineResult.success) {
      await invitePage.screenshot({ path: path.join(SCREENSHOTS_DIR, "child_rthb_offline_registered.png"), fullPage: true }).catch(() => {})
      console.log(`Child (RTHB, OFFLINE) ${childOfflineResult.firstName} ${childOfflineResult.surname} registered while offline. Screenshot: child_rthb_offline_registered.png`)
    } else {
      console.log("Child (RTHB, OFFLINE) registration did not complete — see log above for the step it stalled on.")
    }

    // ----- Record a visit for that same child, still offline (in-app nav only) -----
    if (childOfflineResult.success) {
      await recordVisitOffline(invitePage, {
        personFirstName: childOfflineResult.firstName,
        resultLabel: "Visit (Child RTHB, OFFLINE)",
        screenshotPrefix: "child_rthb_offline_visit",
      })
    } else {
      console.log("Skipping offline visit — folder was not created successfully.")
    }

    // ----- Folder 2 for this test: Pregnant mom, WITH MCR (Maternal Case
    // Record) details, created OFFLINE, then a visit recorded for her too. -----
    // recordVisitOffline leaves us on the child's profile "Visits" tab (after
    // clicking "Back to client profile"). To open a fresh folder from there we
    // need to get back to the Clients list first — using invitePage.goBack(),
    // which pops the SPA's own client-side route/history (no network request,
    // unlike a page.goto() to a URL), not a hard reload.
    async function goBackToClientsListOffline(maxBacks = 3) {
      for (let i = 0; i < maxBacks; i++) {
        const openFolderBtn = invitePage.getByRole("button", { name: /open a folder/i })
        if ((await openFolderBtn.count()) > 0 && (await openFolderBtn.first().isVisible().catch(() => false))) {
          return true
        }
        await invitePage.goBack().catch(() => {})
        await invitePage.waitForTimeout(1500)
      }
      const openFolderBtn = invitePage.getByRole("button", { name: /open a folder/i })
      return (await openFolderBtn.count()) > 0 && (await openFolderBtn.first().isVisible().catch(() => false))
    }

    const backOnClientsList = await goBackToClientsListOffline()
    if (!backOnClientsList) {
      console.log("Could not get back to the Clients list via in-app back navigation — skipping Pregnant mom folder")
    } else {
      console.log("Back on the Clients list (in-app nav) — opening a new Pregnant mom folder, offline")

      // Same two-step tour popup pattern can reappear on this list; dismiss if so.
      const listSkipTourBtn = invitePage.getByText("No, skip", { exact: true })
      if ((await listSkipTourBtn.count()) > 0 && (await listSkipTourBtn.isVisible().catch(() => false))) {
        await listSkipTourBtn.click().catch(() => {})
        await invitePage.waitForTimeout(1000)
      }
      const listCloseTourBtn = invitePage.getByRole("button", { name: "Close" })
      if ((await listCloseTourBtn.count()) > 0 && (await listCloseTourBtn.isVisible().catch(() => false))) {
        await listCloseTourBtn.click().catch(() => {})
        await invitePage.waitForTimeout(1000)
      }

      await invitePage.getByRole("button", { name: /open a folder/i }).first().click({ force: true }).catch(() => {})
      await invitePage.waitForTimeout(2000)
      await openNewFolder("Pregnant mom")

      const momOfflineResult = await runRegistrationWizard(invitePage, {
        label: "Pregnant mom registration (MCR, OFFLINE)",
        generatedId: offlineId,
        CHW: offlineCHW,
        namePrefix: "MomOfflineMCR",
        surnamePrefix: "Auto",
        photoQuestionChoice: "Yes",
      })

      if (momOfflineResult.success) {
        await invitePage.screenshot({ path: path.join(SCREENSHOTS_DIR, "pregnant_mom_mcr_offline_registered.png"), fullPage: true }).catch(() => {})
        console.log(`Pregnant mom (MCR, OFFLINE) ${momOfflineResult.firstName} ${momOfflineResult.surname} registered while offline. Screenshot: pregnant_mom_mcr_offline_registered.png`)

        await recordVisitOffline(invitePage, {
          personFirstName: momOfflineResult.firstName,
          resultLabel: "Visit (Pregnant mom MCR, OFFLINE)",
          screenshotPrefix: "pregnant_mom_mcr_offline_visit",
        })
      } else {
        console.log("Pregnant mom (MCR, OFFLINE) registration did not complete — see log above for the step it stalled on. Skipping her visit.")
      }
    }

    // ----- Only now reconnect: go back online after both offline visits are
    // done, so we can see whether the app syncs its offline-recorded data
    // cleanly. -----
    await context.setOffline(false)
    console.log("Context is back ONLINE (context.setOffline(false)) — checking for sync")
    await invitePage.waitForTimeout(3000)
    await invitePage.screenshot({ path: path.join(SCREENSHOTS_DIR, "offline_test_back_online.png"), fullPage: true }).catch(() => {})
    console.log("Screenshot saved as offline_test_back_online.png")

    // ===== Rest of the code: join the community (moved here, after the offline
    // folder/visit, so that flow no longer waits on it) =====
    await invitePage.goto("https://growgreat-qa-fe.azurewebsites.net/", { waitUntil: "domcontentloaded" })
    await invitePage.waitForTimeout(2000)
    ...
    if ((await communityBioInput.count()) > 0 && (await communityBioInput.isVisible().catch(() => false))) {
      await communityBioInput.fill(`Automated tester ${offlineId}`).catch(() => {})
    }
    await invitePage.getByText("Yes", { exact: true }).click({ force: true }).catch(() => {})
    await invitePage.waitForTimeout(500)
    ...
    console.log("CHW joined community (post-offline step)")
  } catch (error) {
    console.error("An error occurred:", error)
    await page.screenshot({ path: path.join(SCREENSHOTS_DIR, "offline_test_error_screenshot.png"), fullPage: true }).catch(() => {})
    if (invitePage) {
      await invitePage.screenshot({ path: path.join(SCREENSHOTS_DIR, "offline_test_invite_error_screenshot.png"), fullPage: true }).catch(() => {})
    }
    console.log("Error screenshot saved as offline_test_error_screenshot.png")
  } finally {
    await browser.close()
    console.log("Browser closed")
  }
})
test("GG/ggacountcreation", async () => {
  const browser = await chromium.launch({ headless: false })
  const context = await browser.newContext()
  const page = await context.newPage()
...
await invitePage.waitForTimeout(5000);
    ...
    // Step 9: Screenshot
    await page.screenshot({ path: path.join(SCREENSHOTS_DIR, "post_click_user.png"), fullPage: true })
    ...
    await invitePage.screenshot({ path: path.join(SCREENSHOTS_DIR, "community_joined.png"), fullPage: true }).catch(() => {})
    ...
    async function reportResult(result, description, screenshotName) {
      if (result.success) {
        await invitePage.screenshot({ path: path.join(SCREENSHOTS_DIR, screenshotName), fullPage: true }).catch(() => {})
    ...
    if (typeof invitePage !== "undefined") {
      await invitePage.screenshot({ path: path.join(SCREENSHOTS_DIR, "invite_error_screenshot.png"), fullPage: true }).catch(() => {})
    }
