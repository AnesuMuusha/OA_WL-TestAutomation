...
async function checkPageState(currentPage, expectedUrlPattern) {
  ...
  if (url.includes('dynadot') || url.includes('expired') || url.includes('view-user')) {
    ...
    await currentPage.screenshot({ path: path.join(SCREENSHOTS_DIR, "unexpected-redirect.png") });
    return false;
  }
  return true;
}
...
test("WL/wlsignupaddpreschool5Child", async () => {
...
// Unique per run so this practitioner never collides with one from a previous
// run (a stale hardcoded name matched an already-onboarded leftover account
// and broke the "Resend Invitation" step further down).
const practitionerId = `BulkSMSWL10-${Date.now()}`;
...
await safeFill(page, 'input[name="firstName"]', practitionerId);
...
await safeFill(page, 'input[name="idNumber"]', practitionerId);
...
const practitionerName = practitionerId;
...
await safeFill(newPage, 'input[name="username"]', practitionerId);
...
await safeFill(newPage, 'input[placeholder="e.g. Nothando_123"]', practitionerId);
...
await newPage.screenshot({ path: path.join(SCREENSHOTS_DIR, "principal-selection-failed.png") });
throw error2;
}
}
...
await newPage.screenshot({ path: path.join(SCREENSHOTS_DIR, "close-failed-final.png") });
...
await safeFill(newPage, 'input[placeholder="First name"]', 'Lisa');
...
await newPage.getByRole('button', { name: 'Fill in the registration form' }).click();
console.log("Clicked Fill in the registration form");

// Check - Accept personal information agreement (check the actual checkbox)
await newPage.locator('input[name="personalInformationAgreementAccepted"]').check();
...
await newPage.locator('button[aria-haspopup="menu"]').first().click();
console.log("Clicked Day dropdown");

// Wait for the menu to appear and click the day value
await newPage.getByText('15').click();
...
// add name
console.log("=== Filling child's first name ===");
await safeFill(newPage, 'input[placeholder="First name"]', 'Lisa1');

// add surname
console.log("=== Filling child's surname ===");
await safeFill(newPage, 'input[placeholder="Surname/Family name"]', 'Jaz1');
...
if (!finalCloseSuccess) {
  ...
  await newPage.screenshot({ path: path.join(SCREENSHOTS_DIR, "final-close-failed.png") });
...
await page.screenshot({ path: path.join(SCREENSHOTS_DIR, "error-main-page.png") });
...
await newPage.screenshot({ path: path.join(SCREENSHOTS_DIR, "error-new-page.png") });
